...
export async function walk() {
  ...
  // 再開地点のインデックスを指定
  const startSIndex = 1; // 再開したい職種のインデックスを指定（0から始まります）
  const startAIndex = 0; // 再開したいエリアのインデックスを指定（0から始まります）
  ...
  while (!isDisabled) {
    try {
      const textExists = await page.evaluate(() => {
        return document.body.innerText.includes('2024年10月31日');
      });
      if (!textExists) {
        console.log("2024年10月31日が見つかりませんでした。次のエリアに進みます");
        break;
      }
      console.log("2024年10月31日が見つかりました。このページを処理します");
