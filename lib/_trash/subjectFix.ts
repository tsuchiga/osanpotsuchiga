...
import subjectMappingRaw from './category.json';
import { category } from './select'; 

// 型定義の追加

// subject.jsonの内容に基づく型定義
interface SubjectMapping {
    [key: string]: string[];
}

// select.tsのindustry配列に基づく型定義
...
// industryMapの型を指定
const industryMap: { [key: string]: string } = industry.reduce((map: { [key: string]: string }, item: IndustryItem) => {
    ...
    try {
        ...
        while (hasMoreData) {
            const { data, error } = await supabase
                .from('Walk')
                .select('Industry, subject') // 'id'を選択しない
            ...
            for (const row of data) {
                ...
                if (newSubjectLabel && newSubjectLabel !== row.subject && !industriesToUpdate.includes(industryValue)) { // 変更が必要な場合
            ...
            industriesToUpdate.forEach(async (industryValue) => {
                ...
                if (newSubjectLabel) {
                    const { error: updateError } = await supabase
                        .from('Walk')
                        .update({ subject: newSubjectLabel })
