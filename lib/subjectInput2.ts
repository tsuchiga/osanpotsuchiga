...
import subjectMappingRaw from './output2.json';
...
// industryMapの型を指定
const industryMap: { [key: string]: string } = category.reduce((map: { [key: string]: string }, item: IndustryItem) => {
    ...
    try {
        ...
        while (hasMoreData) {
            ...
            for (const row of data) {
                const industryValue: string = row.sub_category;
                const newSubjectLabel: string | null = determineSubjectLabel(industryValue);

                if (newSubjectLabel && newSubjectLabel !== row.category && !industriesToUpdate.includes(industryValue)) { // 変更が必要な場合
            ...
            industriesToUpdate.forEach(async (industryValue) => {
                ...
                if (newSubjectLabel) {
                    const { error: updateError } = await supabase
                        .from('Walk')
                        .update({ category: newSubjectLabel })
                        .eq('sub_category', industryValue);
