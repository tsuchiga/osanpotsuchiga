...
export async function POST(req: NextRequest) {
  try {
          ...
          for (const row of data) {
              ...
              if (!lowerUrl.startsWith('http')) {
                  ...
                  if (updateError) {
                      ...
                      console.log(`${row.id}(${url}) を更新しました: ${updatedUrl}`);
                  }
              }
          };
          from += pageSize;
              ...
              if (url.startsWith('http://')) {
                  ...
                  if (indexOfDoubleSlash !== -1) {
                      ...
                      if (updateError) {
                          ...
                          console.log(`${row.id}(${url}) のURLを修正しました: ${newUrl}`);
                      }
                  }
              }
          };
    ...
    await analyze(); 
    return NextResponse.json({ message: 'Playwright script executed successfully' }, { status: 200 });
  } catch (error) {
    console.error(error);
